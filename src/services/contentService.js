...
function createContent(userId, { type, title, body, metadata, storageKey, mimeType, size }) {
...
function listContent(userId, query) {
  const limit = Math.min(Number(query.limit) || 50, 100);
  const offset = Math.max(Number(query.offset) || 0, 0);
  return contentRepo.listByUser(userId, { limit, offset }).map(contentRepo.toPublic);
}

function getContent(userId, id) {
  const row = contentRepo.findById(id);
  if (!row || row.user_id !== userId) throw new AppError('Content not found', 404, 'NOT_FOUND');
  return contentRepo.toPublic(row);
}

function deleteContent(userId, id) {
  const result = contentRepo.remove(id, userId);
